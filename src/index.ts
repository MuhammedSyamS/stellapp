...
http.createServer(async (_req, res) => {
  ...
  if (_req.method === "GET" && parsedUrl.pathname === "/api/auth/me") {
    res.setHeader("Access-Control-Allow-Origin", ALLOWED_ORIGIN);
    ...
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      res.writeHead(401, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ success: false, error: "Unauthorized: Missing Authorization header" }));
      return;
    }
    ...
    if (!decoded || !decoded.phoneNumber) {
      res.writeHead(401, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ success: false, error: "Unauthorized: Invalid or expired token" }));
      return;
    }

    try {
      const user = await prisma.user.findFirst({
        where: { chatId: `${decoded.phoneNumber.replace(/\D/g, "")}@c.us` },
      ...
      const { getBalances } = require("./services/stellar");
  ...
  if (_req.method === "OPTIONS" && (parsedUrl.pathname?.startsWith("/api/auth") || parsedUrl.pathname?.startsWith("/api/user"))) {
    res.setHeader("Access-Control-Allow-Origin", ALLOWED_ORIGIN);
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization");
